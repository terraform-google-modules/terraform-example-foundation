...
| essential\_contacts | Essential Contacts configuration object:<br>- language: The preferred language for notifications, as a ISO 639-1 language code. See [Supported languages](https://cloud.google.com/resource-manager/docs/managing-notification-contacts#supported-languages) for a list of supported languages.<br>- enable\_technical\_incidents: See [Assigning contacts for outage and disruption notifications](https://cloud.google.com/resource-manager/docs/managing-notification-contacts#technical-incidents-contacts) for conditions to use Technical Incidents. | <pre>object({<br>    language                   = string<br>    enable_technical_incidents = bool<br>  })</pre> | <pre>{<br>  "enable_technical_incidents": false,<br>  "language": "en"<br>}</pre> | no |
...
| group\_billing\_admins | Google Group for GCP Billing Administrators. Should be the same at 0-bootstrap step. | `string` | n/a | yes |
| group\_org\_admins | Google Group for GCP Organization Administrators. Should be the same at 0-bootstrap step. | `string` | n/a | yes |
