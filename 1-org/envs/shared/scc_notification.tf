...
module "scc_notification" {
  ...
  create_cmd_body       = <<-EOF
    alpha scc notifications create ${var.scc_notification_name} --organization ${module.constants.values.org_id} \
...
EOF
  ...
  alpha scc notifications delete organizations/${module.constants.values.org_id}/notificationConfigs/${var.scc_notification_name} \
  --impersonate-service-account ${var.terraform_service_account} \
  --project "${module.scc_notifications.project_id}" \
  --quiet
  EOF
